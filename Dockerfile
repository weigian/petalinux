...
# run the install
RUN sudo chmod a+rx /${PETA_RUN_FILE} && \
  sudo chmod a+rx /accept-eula.sh && \
  sudo mkdir -p /opt/Xilinx && \
  sudo chmod 777 /tmp /opt/Xilinx && \
  cd /tmp && \
...
RUN echo "source /opt/Xilinx/petalinux/settings.sh" >> /home/vivado/.bashrc  && \
    mv /opt/Xilinx/petalinux/tools/hsm/bin/unwrapped/lnx64.o/rlwrap /opt/Xilinx/petalinux/tools/hsm/bin/unwrapped/lnx64.o/rlwrap.00 && \
    ln -fs /usr/bin/rlwrap /opt/Xilinx/petalinux/tools/hsm/bin/unwrapped/lnx64.o/rlwrap
